...
(function($) {
	...
	jQuery.fn.val=function(theValue) {
	    if(theValue!=undefined) {
	        return this.rVal(theValue);
	    }
	    if(this[0]) {
	    	var ele=$(this[0]);
	        if(this.attr('class') && this.attr('class').indexOf('ui-loxia-money') > -1) {
	...
	/* ================================================================================= */
	var loxiaMoney = $.extend({}, loxiaInput, {	
		decimal: function(value){
			if(value == undefined) return this.option("decimal");
			value = "0"||value;
			value = parseInt(value,10);
		...
		_initInput : function(){
			this.element.addClass("ui-loxia-money");
			
			if(this.option("checkmaster"))
				this.option("checkmaster","checkLoxiaMoney," + this.option("checkmaster"));
			else
				this.option("checkmaster","checkLoxiaMoney");
			...
			if(!isNaN(minv)) {
				this.option("min",minv);
			} else {
				minv = -999999999;
			}
			
			if(!isNaN(maxv)) {
				this.option("max",maxv);
			} else {
				maxv = 999999999;
			}
			
			
			$(this.element).autoNumeric('init', {
				aSep: '.',
				aDec: ',',
				mDec: '0'
	...
	$.widget("ui.loxiamoney", loxiaMoney); 
	$.ui.loxiamoney.prototype.options = loxia.defaults;
